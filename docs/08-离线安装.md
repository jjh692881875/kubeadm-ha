...
- 下载所需离线包
  ...
  # 集群运行所需的镜像
  curl -LO https://oss.choerodon.com.cn/kubeadm-ha/kubernetes-1.30.7-images-amd64.tgz
  # CentOS 7 系统对应软件包
  curl -LO https://oss.choerodon.com.cn/kubeadm-ha/centos7/kubernetes-1.30.7-centos7-amd64.tgz
  # 克隆脚本
...
- 准备 kubeadm-ha(ansible) 环境
  - 解压离线包
    ```shell
    tar -xzvf kubernetes-1.30.7-images-amd64.tgz
    tar -xzvf kubernetes-1.30.7-centos7-amd64.tgz
    ```
  - CentOS 类系统添加软件源
