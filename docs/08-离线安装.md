...
- 下载所需离线包
  ...
  # 二进制基础软件包
  curl -LO https://files.saas.hand-china.com/kubeadm-ha/kubernetes-1.33.10-base-amd64.tgz
  # 集群运行所需的镜像
  curl -LO https://files.saas.hand-china.com/kubeadm-ha/kubernetes-1.33.10-images-amd64.tgz
  # CentOS 7 系统对应软件包
  curl -LO https://files.saas.hand-china.com/kubeadm-ha/centos7/kubernetes-1.33.10-centos7-amd64.tgz
  # 克隆脚本
...
- 准备 kubeadm-ha(ansible) 环境
  - 解压离线包
    ```shell
    tar -xzvf kubernetes-1.33.10-base-amd64.tgz
    tar -xzvf kubernetes-1.33.10-images-amd64.tgz
    tar -xzvf kubernetes-1.33.10-centos7-amd64.tgz
    ```
  - CentOS 类系统添加软件源
...
- 推送镜像到已有镜像库（可选项，可跳过）
    ...
    - 如果你不知道这一步可以干什么，请立即跳过
      ...
      sync-images harbor.custom.io admin Harbor12345
