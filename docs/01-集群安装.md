...
- 搭建集群后有以下两种“样式”显示，请自行选择：
  - 样式一
    ...
    192.168.56.11   Ready    control-plane,etcd,master,worker   7m25s   v1.30.7
    192.168.56.12   Ready    control-plane,etcd,master,worker   5m18s   v1.30.7
    192.168.56.13   Ready    control-plane,etcd,master,worker   5m18s   v1.30.7
    192.168.56.14   Ready    worker                             4m37s   v1.30.7
    ```

  - 样式二
    ```
    NAME    STATUS   ROLES                              AGE     VERSION
    node1   Ready    control-plane,etcd,master,worker   7m25s   v1.30.7
    node2   Ready    control-plane,etcd,master,worker   5m18s   v1.30.7
    node3   Ready    control-plane,etcd,master,worker   5m18s   v1.30.7
    node4   Ready    worker                             4m37s   v1.30.7
    ```
